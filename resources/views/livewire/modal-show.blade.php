                            ...
                            <div class="mt-2">
                                <p class="text-sm text-gray-500 mb-2">{{$especie->descripcion}}</p>
                                 @foreach ($especie->tipos as $tipo)
                                    <span class="p-1 mr-2 {{$this->color($tipo->id)}} rounded-xl text-white"> {{$tipo->nombre}} </span>
                                 @endforeach
                            </div>
                        </div>
                    </div>
    ...
    @push('scripts')
        <script>
            document.addEventListener('livewire:load', function() {
                Livewire.find('{{ $this->id }}').then((component) => {
                    ...
                    component.chart = new Chart(ctx, {
                        ...
                        data: component.chart.data,
                        options: component.chart.options
                    });
                });
            });
